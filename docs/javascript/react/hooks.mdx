...
[`useRef`](https://react.dev/reference/react/useRef) is a React Hook that lets you reference a value that’s not needed for rendering. It can be used to access and interact with DOM elements directly, or to store mutable values that don't trigger re-renders.
...
`useReducer` can be combined with `useContext` to create a global state management solution, similar to Redux, but built with React's built-in hooks.
